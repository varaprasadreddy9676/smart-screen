...
const CLICK_VELOCITY_THRESHOLD = 0.005; // normalized units / second
...
export function analyzeInteractions(
  ...
  while (i < telemetry.length - 1) {
    ...
    // --- Click: cursor moves, stops briefly (dwell), then moves again ---
    if (
      vBefore > CLICK_VELOCITY_THRESHOLD &&
      vAfter < CLICK_VELOCITY_THRESHOLD
    ) {
      // Scan forward to find where cursor starts moving again
      let dwellEnd = i + 1;
      while (
        dwellEnd < telemetry.length - 1 &&
        velocity(telemetry[dwellEnd - 1], telemetry[dwellEnd]) < CLICK_VELOCITY_THRESHOLD
      ) {
        dwellEnd++;
      }

      // Use the last still sample as the dwell endpoint
      const dwellEndIdx = Math.min(dwellEnd, telemetry.length - 1);
      const dwellMs = telemetry[dwellEndIdx].timeMs - curr.timeMs;
    ...
    if (vBefore < TYPING_STILLNESS_THRESHOLD && vAfter < TYPING_STILLNESS_THRESHOLD) {
      let stillEnd = i + 1;
      while (
        stillEnd < telemetry.length - 1 &&
        velocity(telemetry[stillEnd - 1], telemetry[stillEnd]) < TYPING_STILLNESS_THRESHOLD
      ) {
        stillEnd++;
      }

      const stillEndIdx = Math.min(stillEnd, telemetry.length - 1);
      const stillMs = telemetry[stillEndIdx].timeMs - curr.timeMs;

      if (stillMs >= TYPING_MIN_DURATION_MS) {
        // Avoid duplicate typing events at the same timestamp
        const lastEvent = events[events.length - 1];
        if (!lastEvent || lastEvent.type !== "typing" || Math.abs(lastEvent.timestamp - curr.timeMs / 1000) > 2) {
          events.push({
            type: "typing",
            x: curr.cx,
            y: curr.cy,
    ...
    // --- Navigation: fast cursor sweep covering significant screen distance ---
