...
export function AnnotationOverlay({
	...
	const width = Math.max(24, (annotation.size.width / 100) * containerWidth);
	const height = Math.max(24, (annotation.size.height / 100) * containerHeight);
	const x = Math.min(
		Math.max(0, (annotation.position.x / 100) * containerWidth),
		Math.max(0, containerWidth - width),
	);
	const y = Math.min(
		Math.max(0, (annotation.position.y / 100) * containerHeight),
	...
	return (
		<Rnd
			...
			onResizeStop={(_e, _direction, ref, _delta, position) => {
				const nextWidth = Math.min(ref.offsetWidth, containerWidth - position.x);
				const nextHeight = Math.min(ref.offsetHeight, containerHeight - position.y);
				const xPercent = (Math.max(0, position.x) / containerWidth) * 100;
				const yPercent = (Math.max(0, position.y) / containerHeight) * 100;
				const widthPercent = (nextWidth / containerWidth) * 100;
				const heightPercent = (nextHeight / containerHeight) * 100;
				onPositionChange(annotation.id, { x: xPercent, y: yPercent });
