...
import { CursorClickPulseOverlay } from "./CursorClickPulseOverlay";
import { KeystrokeOverlay } from "./KeystrokeOverlay";
import { TranscriptCaptionOverlay } from "./TranscriptCaptionOverlay";
import {
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
	(
		...
		const activeCaptionCue = useMemo(
			() => getActiveSubtitleCue(transcriptSegments, Math.round(currentTime * 1000)),
		...
		const activeClickPulse = useMemo(
			() =>
				cursorClickPulseSettings
					...
					: null,
			[cursorTelemetry, currentTime, cursorClickPulseSettings],
		);
		const activeKeystrokeCue = useMemo(
			() =>
				keystrokeOverlaySettings
					...
					: null,
			[keystrokeTelemetry, currentTime, keystrokeOverlaySettings],
		);
		const activeClickPulsePoint = useMemo(() => {
			if (!activeClickPulse || !overlayRef.current) {
				return null;
			}

			const sourceVideoSize =
			...
			return projectCursorPointToStage({
				point: activeClickPulse,
				stageSize: {
				...
				focus: {
					cx: animationStateRef.current.focusX,
					cy: animationStateRef.current.focusY,
				},
			});
		}, [activeClickPulse, currentTime]);
		...
		useEffect(() => {
			...
			const ticker = () => {
				...
				if (region && strength > 0 && !shouldShowUnzoomedView) {
					...
					const regionFocus = clampFocusToScaleUtil(region.focus, zoomScale, stageSizeRef.current);
			...
			>
				...
				{pixiReady && videoReady && (
					...
					>
						...
						{captionSettings?.showInPreview && activeCaptionCue?.text && (
							<TranscriptCaptionOverlay
								text={activeCaptionCue.text}
								settings={captionSettings}
								containerHeight={overlayRef.current?.clientHeight || 600}
							/>
						)}
						{cursorClickPulseSettings?.showInPreview &&
							activeClickPulse &&
							activeClickPulsePoint && (
								<CursorClickPulseOverlay
									point={activeClickPulsePoint}
									progress={activeClickPulse.progress}
									settings={cursorClickPulseSettings}
									containerHeight={overlayRef.current?.clientHeight || 600}
								/>
							)}
						{keystrokeOverlaySettings?.showInPreview && activeKeystrokeCue?.text && (
							<KeystrokeOverlay
								text={activeKeystrokeCue.text}
